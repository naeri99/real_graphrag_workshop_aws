...
def get_all_staff():
    ...
    query = """
    MATCH (s:STAFF)
    RETURN s.name AS name, s.description AS description, s.summary AS summary, s.neptune_id AS neptune_id, labels(s) AS entity_type
    """
    return execute_cypher(query)
...
def get_all_reviewers():
    ...
    query = """
    MATCH (r:REVIEWER)
    RETURN r.id AS id, r.name AS name, r.neptune_id AS neptune_id, labels(r) AS entity_type
    """
    return execute_cypher(query)
