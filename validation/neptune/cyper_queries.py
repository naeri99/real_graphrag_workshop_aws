...
def import_nodes_with_dynamic_label(entities, movie_id, reviewer_id, chunk_id, text):
    ...
    """
    # First, create the base structure (REVIEWER, MOVIE, Chunk) with neptune_id
...
def save_entity_summary(entity_name, summary, entity_type=None):
    ...
    Updates the entity's summary property.
    
    Args:
        entity_name: Name of the entity
        summary: Summary text to save
        entity_type: Optional entity type/label to filter by
    """
    if entity_type:
        query = f"""
        MATCH (n:{entity_type})
...
def save_relationship_summary(source_entity, target_entity, summary, source_type=None, target_type=None):
    """
    Save relationship summary between two entities.
    Creates or updates SUMMARIZED_RELATIONSHIP.
    
    Args:
        source_entity: Name of the source entity
        target_entity: Name of the target entity
        summary: Summary text to save
        source_type: Optional entity type/label for source entity
        target_type: Optional entity type/label for target entity
    """
    # Ensure consistent direction by ordering entity names alphabetically
    if source_entity > target_entity:
        source_entity, target_entity = target_entity, source_entity
        source_type, target_type = target_type, source_type
    
    # Build query based on whether entity types are provided
    ...
    return execute_cypher(query, source_entity=source_entity, 
                         target_entity=target_entity, summary=summary)
...
def get_all_entities_for_summary():
    """Get all entities that need summarization (entities with multiple descriptions)."""
    query = """
    MATCH (n)
    WHERE n.name IS NOT NULL AND n.description IS NOT NULL
    AND size(split(n.description, ',')) > 1
    RETURN n.name AS name, n.description AS description, labels(n) AS entity_type
    """
    return execute_cypher(query)
...
def get_all_reviewers():
    ...
    query = """
    MATCH (r:REVIEWER)
    RETURN r.id AS id, properties(r) AS properties
    """
    return execute_cypher(query)
...
def get_all_summarized_relationships():
    """Get all summarized relationships between entities."""
    query = """
    MATCH (s)-[r:SUMMARIZED_RELATIONSHIP]->(t)
...
def get_all_relationships_for_summary():
    """Get all relationships that need summarization (all relationships)."""
    query = """
    MATCH (s)-[r:RELATIONSHIP]-(t)
    WHERE id(s) < id(t)
    WITH s.name AS source, t.name AS target, labels(s) AS source_type, labels(t) AS target_type, 
         collect(r.description) AS description_list, count(*) AS count
    RETURN source, target, source_type, target_type, description_list, count
    ORDER BY count DESC
    """
    return execute_cypher(query)
...
def find_duplicate_relationships():
    """Find relationships that have duplicates, including bidirectional duplicates (A-B and B-A)."""
    query = """
    MATCH (s)-[r:RELATIONSHIP]->(t)
    WITH s, t, r
    WHERE s.name <> t.name
    WITH 
        CASE WHEN s.name < t.name THEN s.name ELSE t.name END AS entity1,
        CASE WHEN s.name < t.name THEN t.name ELSE s.name END AS entity2,
        collect(r.description) AS descriptions
    WITH entity1, entity2, descriptions, size(descriptions) AS rel_count
    WHERE rel_count > 1
    RETURN entity1, entity2, descriptions, rel_count
...
def cleanup_duplicate_relationships():
    """
    Clean up ALL duplicate relationships including bidirectional ones.
    """
    # Get all relationships and group by entity pairs
    query = """
    MATCH (s)-[r:RELATIONSHIP]->(t)
    RETURN s.name AS source, t.name AS target, labels(s)[0] AS source_type, labels(t)[0] AS target_type, 
           r.description AS description, r.strength AS strength
    """
    
    all_rels = execute_cypher(query)
    if not all_rels or 'results' not in all_rels:
        return []
    
    # Group by normalized entity pairs
    relationship_groups = {}
    for rel in all_rels['results']:
        source = rel['source']
        target = rel['target']
        ...
        if source < target:
            key = (source, target, source_type, target_type)
        else:
            key = (target, source, target_type, source_type)
        
        if key not in relationship_groups:
            relationship_groups[key] = []
        relationship_groups[key].append({'description': description, 'strength': strength})
    
    # Process only groups with duplicates
    results = []
    for (entity1, entity2, type1, type2), rels in relationship_groups.items():
        if len(rels) <= 1:
            continue
            ...
            if isinstance(desc, str):
                try:
                    ...
                    if isinstance(parsed, list):
                        all_descriptions.extend(parsed)
                    else:
                        all_descriptions.append(parsed)
                except:
                    all_descriptions.append(desc)
            elif isinstance(desc, list):
                all_descriptions.extend(desc)
            
            max_strength = max(max_strength, float(rel.get('strength', 0)))
        ...
        # Delete ALL relationships between these entities (both directions)
        delete_query = f"""
        MATCH (a)-[r:RELATIONSHIP]-(b)
        WHERE (a.name = $entity1 AND b.name = $entity2) OR (a.name = $entity2 AND b.name = $entity1)
        DELETE r
        ...
        result = execute_cypher(create_query, entity1=entity1, entity2=entity2,
                               descriptions=descriptions_str, strength=max_strength)
        results.append(result)
...
def find_bidirectional_relationships():
    """Find A-B and B-A bidirectional relationship pairs."""
    query = """
    MATCH (a)-[r1:RELATIONSHIP]->(b)
    MATCH (b)-[r2:RELATIONSHIP]->(a)
    WHERE a.name < b.name
    RETURN a.name AS entity1, b.name AS entity2, 
           r1.description AS forward_desc, r2.description AS backward_desc
    """
    return execute_cypher(query)


def count_relationship_directions():
    """Count relationships by direction to identify potential duplicates."""
    query = """
    MATCH (s)-[r:RELATIONSHIP]->(t)
    WITH 
        CASE WHEN s.name < t.name THEN s.name ELSE t.name END AS entity1,
        CASE WHEN s.name < t.name THEN t.name ELSE s.name END AS entity2,
        CASE WHEN s.name < t.name THEN 'forward' ELSE 'backward' END AS direction
    WITH entity1, entity2, collect(direction) AS directions, count(*) AS total_count
    WHERE size(directions) > 1 OR total_count > 1
    RETURN entity1, entity2, directions, total_count
...
def merge_specific_relationship(source_name, target_name):
    """Merge relationships between two specific entities."""
    query = """
    MATCH (s)-[r:RELATIONSHIP]-(t)
    WHERE s.name = $source_name AND t.name = $target_name
    WITH s, t, collect(r.description) AS descriptions, collect(r) AS rels
    
    // Delete old relationships
    FOREACH (rel in rels | DELETE rel)
    
    // Create new merged relationship with combined description
    CREATE (s)-[new_r:RELATIONSHIP {
        description: reduce(combined = '', desc IN descriptions | 
            CASE WHEN combined = '' THEN desc ELSE combined + '; ' + desc END),
        strength: 1,
        merged: true,
        original_count: size(rels)
    }]-(t)
    
    RETURN s.name AS source, t.name AS target, size(rels) AS merged_count, new_r.description AS new_description
    """
    return execute_cypher(query, source_name=source_name, target_name=target_name)
...
def compare_relationship_counts():
    """Compare RELATIONSHIP vs SUMMARIZED_RELATIONSHIP counts."""
    query = """
    MATCH ()-[r]->()
    WHERE type(r) IN ['RELATIONSHIP', 'SUMMARIZED_RELATIONSHIP']
    RETURN type(r) AS relationship_type, count(r) AS count
...
def delete_all_nodes():
    """Delete all nodes using DETACH DELETE (removes relationships automatically)."""
    query = """
    MATCH (n)
    DETACH DELETE n
    """
...
# convert_summarized_relationship = """
...
# basic_entity = """
# CREATE CONSTRAINT ON (m:MOVIE) ASSERT m.unique_id IS UNIQUE;
# CREATE CONSTRAINT ON (s:MOVIE_STAFF) ASSERT s.unique_id IS UNIQUE;
# CREATE CONSTRAINT ON (c:MOVIE_CHARACTER) ASSERT c.unique_id IS UNIQUE;
# CREATE CONSTRAINT ON (r:REVIEWER) ASSERT r.unique_id IS UNIQUE;
# CREATE INDEX ON :MOVIE(unique_id);
# CREATE INDEX ON :MOVIE_STAFF(unique_id);
# CREATE INDEX ON :MOVIE_CHARACTER(unique_id);
# CREATE INDEX ON :REVIEWER(unique_id);
# """
